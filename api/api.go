...
SELECT m AS month,
       COALESCE((
         SELECT COUNT(*)
         FROM accounting_books ab
         WHERE EXTRACT(YEAR FROM ab.date_issue)::int = $1::int
           AND EXTRACT(MONTH FROM ab.date_issue)::int = m
       ),0) AS issues
FROM months
ORDER BY m;
`
...
WHERE b.year_publication = (
  SELECT MIN(b2.year_publication) FROM books b2 WHERE b2.reading_room_id = rr.id
)
...
func (a *API) reportRoomsByGroups(w http.ResponseWriter, r *http.Request) {
	raw := strings.TrimSpace(r.URL.Query().Get("groups"))
	if raw == "" {
		writeJSON(w, []string{})
		return
	}
...
WHERE NOT EXISTS (
  SELECT 1
  FROM books b
  WHERE b.reading_room_id = rr.id
    AND b.book_group_id NOT IN (%s)
)
ORDER BY rr.name;
`, inList)
...
func (a *API) reportTop5LastMonth(w http.ResponseWriter, r *http.Request) {
	q := `
WITH period AS (
  SELECT date_trunc('month', now()) - interval '1 month' AS start_dt,
         date_trunc('month', now())                         AS end_dt
)
SELECT b.name AS title, a.name AS author, COUNT(*) AS cnt
FROM accounting_books ab
JOIN books b ON b.id = ab.book_id
JOIN book_authors a ON a.id = b.author_id, period p
WHERE ab.date_issue >= p.start_dt::date AND ab.date_issue < p.end_dt::date
...
`
	...
	for rows.Next() {
		var rr row
		if err := rows.Scan(&rr.Title, &rr.Author, &rr.Count); err != nil {
			bad(w, err, 500)
			return
		}
		out = append(out, rr)
